package com.polovyi.ivan.tutorials.utils;

import static com.polovyi.ivan.tutorials.utils.Customer.CustomerType.BUSINESS;
import static com.polovyi.ivan.tutorials.utils.Customer.CustomerType.INDIVIDUAL;

import com.polovyi.ivan.tutorials.utils.Customer.Address;
import java.lang.reflect.Field;
import java.time.LocalDate;
import java.util.List;
import java.util.Set;
import java.util.stream.Collectors;
import lombok.SneakyThrows;

public class FakeCustomersDao {

    public static List<Customer> retrieveCustomers() {
        return List.of(
                new Customer(1,
                        "Lorraine M. Brown",
                        "17737278341",
                        INDIVIDUAL,
                        false,
                        Set.of("VISA", "MASTER", "AMEX", "CASH"),
                        new Address(
                                "Chicago",
                                "IL"
                        ),
                        50L,
                        LocalDate.of(2018, 3, 1)
                ),
                new Customer(2,
                        "Harold N. Robichaux",
                        "18137278341",
                        INDIVIDUAL,
                        true,
                        Set.of("VISA", "MASTER"),
                        new Address(
                                "Tampa",
                                "FL"
                        ),
                        100L,
                        LocalDate.of(2019, 4, 1)
                ),
                new Customer(3,
                        "Jesus L. Devoe",
                        "18137278341",
                        INDIVIDUAL,
                        true,
                        Set.of("VISA", "MASTER", "AMEX", "CASH", "DISCOVER"),
                        new Address(
                                "Gainesville",
                                "FL"
                        ),
                        25L,
                        LocalDate.of(2019, 5, 1)
                ),
                new Customer(4,
                        "Reynolds PLC",
                        "16057278341",
                        BUSINESS,
                        true,
                        Set.of("VISA", "DISCOVER"),
                        new Address(
                                "Sioux Falls",
                                "SD"
                        ),
                        150L,
                        LocalDate.of(2020, 6, 1)
                ),
                new Customer(5,
                        "Spencer Group",
                        "17737278341",
                        BUSINESS,
                        false,
                        Set.of("VISA", "AMEX"),
                        new Address(
                                "Chicago Ridge",
                                "IL"
                        ),
                        130L,
                        LocalDate.of(2019, 6, 1)
                )
        );
    }

    public static List<Customer> retrieveCustomers(Set<String> fieldNames) {
        return retrieveCustomers().stream()
                .map(c -> {
                    Class<Customer> customerClass = Customer.class;
                    Field[] declaredFields = customerClass.getDeclaredFields();
                    Set<Field> nullFields = List.of(declaredFields).stream()
                            .filter(f -> !fieldNames.contains(f.getName()))
                            .collect(Collectors.toSet());
                    nullFields.forEach(f -> setField(c, f));
                    return c;
                }).collect(Collectors.toList());
    }

    @SneakyThrows
    private static void setField(Customer c, Field f) {
        f.setAccessible(true);
        f.set(c, null);
    }
}
